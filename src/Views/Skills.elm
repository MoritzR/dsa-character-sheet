...
import Model exposing (Character, Message, Model, Skills)
...
view model =
    let
        ...
        skill updater name bonus against =
            div []
                [ span []
                    [ text (name ++ ": ")
                    , editableField model updater (String.fromInt bonus)
                    ]
...
updateClimb =
    update .climb (\s v -> { s | climb = v })


updateBodyControl =
    update .bodyControl (\s v -> { s | bodyControl = v })


updateComposure =
    update .composure (\s v -> { s | composure = v })


updateAcuity =
    update .acuity (\s v -> { s | acuity = v })


updatePickpocket =
    update .pickpocket (\s v -> { s | pickpocket = v })


updateHide =
    update .hide (\s v -> { s | hide = v })


updateAlleyLore =
    update .alleyLore (\s v -> { s | alleyLore = v })


updateInsight =
    update .insight (\s v -> { s | insight = v })


updatePersuade =
    update .persuade (\s v -> { s | persuade = v })


updateLockpicking =
    update .lockpicking (\s v -> { s | lockpicking = v })
...
update defaultGetter setter value character =
    let
        skills =
            character.skills
    in
    { character | skills = setter skills (Maybe.withDefault (defaultGetter skills) (String.toInt value)) }
